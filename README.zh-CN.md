...
四步，而且一路上每个默认值都可以直接接受。

### 1. 下载并解压

从 [Releases](../../releases) 拿到对应你系统的压缩包，解压到任意位置。无需安装 ——
不需要 .NET 运行时，也没有别的依赖。然后运行 `stingray-tex-gui`。

### 2. 打开放 mod 的那个文件夹

点 **Open folder…**，指向你的 mod 管理器存放 mod 的地方，它会把下面所有内容都搜一遍。

![扫描 mod 管理器的文件夹](docs/images/04-scan.zh.png)

每一行都标着这个 mod 占多少显存 —— 这正是整个工具要解决的那个数字。列表就是文件夹树，
因为这是文件本身唯一带有的结构：带可选项的 mod 会把变体放在自己下面的子文件夹里，
于是它们会出现在所属的那个 mod 下面，而不是变成平铺列表里几个看不出来历的名字。
只有一个资源包的 mod 直接显示成一行，不用再点开一层。

如果这个 mod 是你自己做的、而且你只想处理某一个文件，**Open bundle…** 可以直接打开它 ——
就是那个**没有扩展名**、或者以 `.patch_0`、`.patch_1` 之类结尾的文件。不要选
`.gpu_resources`，那个会被自动找到。

### 3. 勾选要处理的，然后点“分析”

勾选一个 mod 就等于勾选它下面的全部，按钮上的数字是还没分析的那些 —— 会显示成
**分析 6 个**，而不是光写一个“分析”。点下去：

![来自两个 mod 的六个资源包，一起分析](docs/images/05-batch.zh.png)

勾选的一切会被读进同一张表。“所属 Mod”一列上面写 mod、下面写变体，因为像 *本体*
这样的名字单独拿出来什么也说明不了。右下角的总计是整个选择的合计 —— 这里是
**571.7 MiB → 228.2 MiB**，六个资源包一次性减掉 60%。

到这一步还没有写入任何东西。分析只是读。

### 4. 看懂方案，然后点 Optimize

每一行是一张纹理。**Size → New** 是它现在占多少、之后会占多少，**Content** 是分析器
在里面发现了什么，左边的勾去掉就能保留那一行不动。

![单独打开一个资源包](docs/images/01-plan.zh.png)

那是单个资源包的样子 —— 点击（而不是勾选）某个 mod 就会得到它，同一张表，只是来源只有
一个。其中两张纹理整张都是同一个颜色，于是从 2048² 折叠成 16×16：各自从 16 MiB 变成
128 字节，而且在任何尺寸下采样结果都一样。按钮上方的横幅报告的是那些出现了不止一次、
将会只存一份的数据。

右下角是总计 **475.1 MiB → 174.7 MiB**，下面单独列着显存 —— 如果你显存紧张，那才是
要盯的数字，因为共享重复数据只缩小文件，不改变显卡里放着的东西。

按 **Optimize**。在写入之前，你的原始文件会先被复制到资源包旁边的 `backup/` 文件夹里，
写完之后结果还会被自动校验。
...
**0.1.3 到底变了什么 —— 因为 0.1.0 到 0.1.2 都没有被报过。** Windows 这边真正有实质
变化的只有一样东西，就是 `stingray_cmp.dll`。在 0.1.3 之前，它因为少了导出标记而没有
导出任何函数，于是链接器把它裁成了 60 KB 的填充 —— 熵 1.8，基本是个空壳。修好导出之后
它才变成本来就该有的样子：208 KB 的向量代码，熵 5.4，`.text` 大了 28 倍。所以对所有
扫描引擎来说，这个库都是第一次见，没有签名，而且在此之前连一条说明它是谁的版本资源
都没有。从 0.1.4 起它带上了。

**如果你干脆不想要它，删掉就行。** 快速编码器是可选的：`stingray_cmp.dll` 不在时，
工具会回退到托管编码器，一切照常，只是慢好几倍。用 `stingray-tex --version` 可以看到
它找到的是哪一个。压缩包里没有别的东西依赖它。

**比起绕开它，上报更有用。** MaxSecure 和其他引擎都接受误报提交，对于一个开源工具，
只有单独一家报警的情况通常在有人反馈之后几天内就会解除。
...
**扫描时会跳过所有 backup 文件夹**，包括这个工具自己写出来的那些。否则你已经优化过的
每个 mod 都会出现两次 —— 一次是现在的样子，一次是原来的样子 —— 而优化错了那一个就会
毁掉你唯一的退路。

**分析结果在还有效的期间会一直留着。** 取消勾选某个 mod，它的行就从表里离开；重新勾上，
它们会原样回来，包括你之前逐张取消勾选的那些纹理。勾选新的东西时，只有新来的那个会被
分析。在一个真实文件夹上，四个资源包要花 2.9 秒，之后再加第五个只要 156 毫秒，
而不是再花 2.9 秒。点击某个 mod 看一眼也算，所以之后再勾选它是不花钱的。

**唯一会作废它的是改动设置**，因为方案描述的是它当时所依据的设置。这时按钮会重新变回
“分析”，而不是直接写入。

**屏幕上显示的永远就是按钮会处理的东西。** 单独打开一个资源包会把批量结果从表里撤下来，
而不是把它的总计和这个资源包的加在一起。半勾状态是一个 mod 由它的变体告诉它的，
永远不会是点击能选出来的。

写入时会逐个备份并校验。结果发现缩小不了的会被计数并原样留下，而不是白写一遍。
