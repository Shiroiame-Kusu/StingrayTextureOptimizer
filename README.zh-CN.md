...
点击某个资源包就能看到它的处理方案，和直接打开它一样。**也可以勾选多个然后点“优化”，
一次把它们全做完** —— 按上面的设置逐个处理，勾选一个 mod 就等于勾选它下面的全部。
这种情况下没有可以先审阅的方案，因为多个资源包本来就没有单一的方案；顶替它的是：
每一个仍然会在写入前备份、写入后校验，而任何缩小不了的都会被报告并跳过。
