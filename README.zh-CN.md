...
> 一进游戏显存就爆了，然后立刻开始卡顿。
>
> 所以我用 AI 帮我做了这个工具。不过所有测试和代码审查仍然是我自己手动完成的。
>
> 我只是懒，不是傻。😎
>
> GLHF！

通过对 Stingray/Bitsquid 游戏资源包（bundle）内的纹理做块压缩来缩小它们。为那些
`.gpu_resources` 已经膨胀到几百 MB 的 mod 作者准备。
...
工具测出 8 张没有可见损失、7 张略微变软、1 张有可见变软 —— 并且会告诉你是哪一张，
于是你可以单独把它保持原尺寸。
...
**2. 指向你的 mod。** 运行 `stingray-tex-gui`，点击 **Open bundle…**，选中资源包文件
...
**分享 mod 之前请先在游戏里测试结果。** 在那之前请保留 `backup/` 文件夹。
...
| 36–45 dB | `slight softening`（略微变软） |
| 30–36 dB | `visible softening`（可见变软） |
...
只有一张 30 dB —— 那是一张法线贴图，它细密的纹路确实有可见的变软。所以“我该不该把
...
条目都能解析到相同的字节 —— 但在你发布之前，这是最值得在游戏里确认的部分。
`--no-dedup` 可以完全避开它。
