...
列表就是文件夹树，因为这是文件本身唯一带有的结构。带可选项的 mod 会把变体放在自己下面的
子文件夹里，所以 *Long boi* 和 *Shorty* 会显示在它们所属的那个 mod 下面，而不是变成
一个平铺列表里两个看不出来历的名字。每一行都标着自己占多大；只有一个资源包的 mod 直接
显示成一行，不用再点开一层。

点击某个资源包就能看到它的处理方案，和直接打开它一样。

**也可以勾选多个 —— 勾选一个 mod 就等于勾选它下面的全部 —— 这时按钮会变成“分析”。**
点一下，所有勾选的资源包都会被分析，它们的纹理会一起显示在同一张表里，并用“所属 Mod”
一列标明每一行来自哪个资源包，右下角的总计是整个选择的合计。只有到这一步按钮才会变成
“优化”，所以批量处理和单个资源包一样，都是先看过再写；你依然可以跨多个资源包逐张取消
勾选。

改动任何设置都会作废这次分析，按钮重新变回“分析” —— 因为方案描述的是它当时所依据的
设置。写入时会逐个备份并校验，缩小不了的会被报告并跳过。
